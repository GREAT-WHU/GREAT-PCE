...
namespace gnut
{
	...
	bool t_gallbias::is_osb() {
		string used_ac = get_used_ac();
		if (used_ac.find("_A") != used_ac.npos) return true;
		else return false;
	}

	void t_gallbias::set_used_ac(string ac)
	{
		_used_ac = ac;
	}
	...
	t_spt_bias t_gallbias::_find(const string& ac, const t_gtime& epo, const string& obj, const GOBS& gobs)
	{
		...
		if (itAC != _mapbias.end()) {
			auto itEPO = itAC->second.upper_bound(epo);
			if (itEPO != itAC->second.begin()) itEPO--;  // between epochs
			if (itEPO == itAC->second.end() && itAC->second.size() != 0) itEPO--;         // no epochs
	...
	// clean rtcm & gnav
	// ----------
	void t_gallbias::clean_outer(const t_gtime& beg, const t_gtime& end)
	{
		gtrace("t_gallbias::clean_outer");
...
#ifdef DEBUG
		if (_log && _log->verb() >= 1) _log->comment(1, "gallbias", "biases clean request: "
		...
		while (itAC != _mapbias.end()) {
			...
			if (itBeg != itFirst) {
				...
				if (itBeg == itLast) {
					itBeg--;
					if (_log && _log->verb() >= 2) _log->comment(2, "gallbias", "biases removed before: "
						+ itFirst->first.str("%Y-%m-%d %H:%M:%S[%T] ") + acs);
					...
					if (_log && _log->verb() >= 2) _log->comment(2, "gallbias", "biases removed before: "
						+ itBeg->first.str("%Y-%m-%d %H:%M:%S[%T] ") + acs);
			...
			if (itEnd != itLast) {
				if (_log && _log->verb() >= 2) _log->comment(2, "gallbias", "biases removed after : "
					+ itEnd->first.str("%Y-%m-%d %H:%M:%S[%T] ") + acs);

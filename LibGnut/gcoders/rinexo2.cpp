  ...
  // -------- "MARKER NAME" --------
    ...
    if( ! _fname.empty() ){
        if (_fname.length() > 8 + 4 && n >= 5) {
            auto pos = filename.find(".rnx");
            if (pos != filename.npos) { guess = filename.substr(pos - 34, 9); src = "FILE"; typ = "long"; sit = "(9-CH)"; }
        } // LONG  FILE NAME
        else {
            guess = filename.substr(filename.find_last_of("o") - 11, 4); src = "FILE"; typ = "short"; sit = "(4-CH)";
        }// SHORT FILE NAME
...
//  char sys( string(1,_rnxhdr.rnxsys());
  ...
  // -------- "TIME OF FIRST OBS" -------- 
	...
	else if (tsys == "BDT"|| tsys == "BDS")   first.tsys(t_gtime::BDS);
  ...
  // -------- "TIME OF LAST OBS" -------- 
	...
	else if (tsys == "BDT" || tsys == "BDS")   last.tsys(t_gtime::BDS);
                         ...
                         t_spt_gobs obs )
{
  ...
  if( _obs[gsys].size() > 0 &&
      _obs[gsys].find( gobs2str(type) ) == _obs[gsys].end() ) return 0;
...
int t_rinexo2::_fix_band(string sys, string& go)
{
  ...
  switch ( gsys ){
    case GLO : if( _version <= "2.12" && go[1] == '7' ){
                 if( _log ) _log->comment(0,"rinexo2","Warning: extended RINEX 2.11 format (GLO L7 skipped)");
                 else               cerr << "rinexo2 - Warning: extended RINEX 2.11 format (GLO L7 skipped)\n";
                                        mesg(GWARNING,"Warning: extended RINEX 2.11 format (GLO L7 skipped)");
                 return 0;
              }; break;

    case GPS : if( _version <= "2.12" && go[1] == '7' ){
                 if( _log ) _log->comment(0,"rinexo2","Warning: extended RINEX 2.11 format (GPS L7 skipped)");
                 else               cerr << "rinexo2 - Warning: extended RINEX 2.11 format (GPS L7 skipped)\n";
                                        mesg(GWARNING,"Warning: extended RINEX 2.11 format (GPS L7 skipped)");
                 return 0;
              }; break;

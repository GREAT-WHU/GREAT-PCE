...
// ----------
double t_gpcv::pco( double zen, double azi, GFRQ f )
{
#ifdef BMUTEX   
...
//    GSYS gsys = t_gsys::gfrq2gsys(f);
//    if(_gnote) _gnote->mesg(GWARNING,"gpcv","no REC PCV for freq["+t_gfreq::gfreq2str(f)+"], substitute GPS L1/L2");
//    f = t_gsys::freq_priority(GPS, (t_gsys::gfrq2freq(gsys,f) <= 2) ? (t_gsys::gfrq2freq(gsys,f)) : FREQ_2 );
//  }   
  // ==================================================
...
int t_gpcv::pcoS_cmb(t_gsatdata & satdata, t_gtriple & pco, GOBSBAND & b1, GOBSBAND & b2)
{
	...
	GFRQ def_f2 = t_gsys::freq_priority(gsys, FREQ_2);

	FREQ_SEQ freq1 = t_gsys::gfrq2freq(gsys, f1);
	FREQ_SEQ freq2 = t_gsys::gfrq2freq(gsys, f2);
	...
	else
	{
		if (_gnote)
		{
			if (_mappco.find(f1) != _mappco.end()) _gnote->mesg(GWARNING, "gpcv", "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "], using hardwired values");
			if (_mappco.find(f2) != _mappco.end()) _gnote->mesg(GWARNING, "gpcv", "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f2) + "], using hardwired values");
		}
...
int t_gpcv::pcoR_cmb(t_gsatdata & satdata, t_gtriple & pco, GOBSBAND & b1, GOBSBAND & b2)
{
	...
	if (_mappco.find(f1) == _mappco.end())
	{
		if (_gnote) _gnote->mesg(GWARNING, "gpcv", "no REC PCO [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "], substituted with GPS L1");
		f1 = t_gsys::freq_priority(gsys, (t_gsys::gfrq2freq(gsys, f1) <= 2) ? t_gsys::gfrq2freq(gsys, f1) : FREQ_2);
	}
	if (_mappco.find(f2) == _mappco.end())
	{
		if (_gnote) _gnote->mesg(GWARNING, "gpcv", "no REC PCO [" + _anten + "/" + t_gfreq::gfreq2str(f2) + "], substituted with GPS L2");
		f2 = t_gsys::freq_priority(gsys, (t_gsys::gfrq2freq(gsys, f2) <= 2) ? t_gsys::gfrq2freq(gsys, f2) : FREQ_2);
	}
	...
	if (_mappco.find(f1) == _mappco.end() || _mappco.find(f2) == _mappco.end())
	{
		if (_gnote)
		{
			if (_mappco.find(f1) == _mappco.end()) { _gnote->mesg(GWARNING, "gpcv", "no REC PCO [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "]"); }
			if (_mappco.find(f2) == _mappco.end()) { _gnote->mesg(GWARNING, "gpcv", "no REC PCO [" + _anten + "/" + t_gfreq::gfreq2str(f2) + "]"); }
		}
...
int t_gpcv::pcvS_cmb(double & corr, t_gsatdata & satdata, GOBSBAND & b1, GOBSBAND & b2, t_gtriple & site)
{
  ...
  // JD: New flexible way of defining L3 frequency for multi-GNSS
	...
	{
		if (_gnote) _gnote->mesg(GWARNING, "gpcv", "no REC PCV [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "], substituted with GSYS L1");
		f1 = t_gsys::freq_priority(gsys, (t_gsys::gfrq2freq(gsys, f1) <= 2) ? t_gsys::gfrq2freq(gsys, f1) : FREQ_2);
	}

	if (_mapzen.find(f2) == _mapzen.end())
	{
		if (_gnote) _gnote->mesg(GWARNING, "gpcv", "no REC PCV [" + _anten + "/" + t_gfreq::gfreq2str(f2) + "], substituted with GSYS L1");
	...
	{
		if (_gnote) { _gnote->mesg(GWARNING, "gpcv", "no SAT PCV [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "]"); }
		_gmutex.unlock();
		return -1;
	}

	map<double, double> mapDataf1;  mapDataf1 = _mapzen[f1];
	map<double, double> mapDataf2;  mapDataf2 = _mapzen[f2];
...
int t_gpcv::pcvR_cmb(double & corr, t_gsatdata & satdata, GOBSBAND & b1, GOBSBAND & b2)
{
	...
	if (_mapzen.find(f1) == _mapzen.end())
	{
		if (_gnote) _gnote->mesg(GWARNING, "gpcv", "no REC PCV [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "], substituted with GPS L1");
		f1 = t_gsys::freq_priority(gsys, (t_gsys::gfrq2freq(gsys, f1) <= 2) ? t_gsys::gfrq2freq(gsys, f1) : FREQ_2);
	}

	if (_mapzen.find(f2) == _mapzen.end())
	{
		if (_gnote) _gnote->mesg(GWARNING, "gpcv", "no REC PCV [" + _anten + "/" + t_gfreq::gfreq2str(f2) + "], substituted with GPS L2");
		f2 = t_gsys::freq_priority(gsys, (t_gsys::gfrq2freq(gsys, f2) <= 2) ? t_gsys::gfrq2freq(gsys, f2) : FREQ_2);
	}
	...
	if (_mapzen.find(f1) == _mapzen.end()) { _gmutex.unlock(); return -1; }
	if (_mapzen.find(f2) == _mapzen.end()) { _gmutex.unlock(); return -1; }
	...
	{  // AZI-dependant calibration NOT available (only NOAZI)
		...
		if (_gnote) { _gnote->mesg(GWARNING, "gpcv", "no REC AZI PCV [" + _anten + "/freq:" + t_gfreq::gfreq2str(f1) + "], just used NOAZI"); }
...
int t_gpcv::pcoR_raw(t_gsatdata & satdata, t_gtriple & pco, GOBSBAND & b1)
{
  ...
  // JD: New flexible way of defining L3 frequency for multi-GNSS
	GFRQ f1 = t_gsys::band2gfrq(gsys, b1);
	FREQ_SEQ freq = t_gsys::gfrq2freq(gsys, f1);

	// fix for muti-GNSS
	if (_mappco.find(f1) == _mappco.end())
	{
		if (_gnote) _gnote->mesg(GWARNING, "gpcv", "no REC PCO [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "], substituted with GPS L2");
		f1 = t_gsys::freq_priority(gsys, (t_gsys::gfrq2freq(gsys, f1) <= 2) ? t_gsys::gfrq2freq(gsys, f1) : FREQ_2);
	}
...
int t_gpcv::pcvS_raw(double & corr, t_gsatdata & satdata, GOBSBAND & b1, t_gtriple & site)
{
  ...
  // JD: New flexible way of defining L3 frequency for multi-GNSS
	...
	// for zenith angle at satellite-side
	double eleS = satdata.ele();
	double rS = satdata.satcrd().crd_cvect().NormFrobenius();
	double rR = site.crd_cvect().NormFrobenius(); 
	double sinz = (rR / rS)*cos(eleS);
	...
	if (_azi_dependent(f1)) {
		...
		for (map<GFRQ, t_map_A>::iterator itGFRQ = _mapazi.begin(); itGFRQ != _mapazi.end(); itGFRQ++)
		{
			...
			map<double, t_map_Z>::iterator itA1 = itGFRQ->second.lower_bound(azi);
			if (itA1 == itGFRQ->second.end() || itA1 == itGFRQ->second.begin()) { return -1; }
			map<double, t_map_Z>::iterator itA2 = itA1; itA2--;

			map<double, double>::iterator itZ1 = itA1->second.lower_bound(zen);
			if (itZ1 == itA1->second.end() || itZ1 == itA1->second.begin()) { return -1; }
			map<double, double>::iterator itZ2 = itZ1; itZ2--;

			map<double, double>::iterator itZ3 = itA2->second.lower_bound(zen);
			if (itZ3 == itA2->second.end() || itZ3 == itA2->second.begin()) { return -1; }
			map<double, double>::iterator itZ4 = itZ3; itZ4--;
...
int t_gpcv::pcvR_raw(double & corr, t_gsatdata & satdata, GOBSBAND & b1)
{
	...
	if (_mapzen.find(f1) == _mapzen.end()) {  return -1; }
...
int t_gpcv::pcoS( t_gsatdata& satdata, t_gtriple& pco, GOBS_LC lc,  GOBSBAND k1, GOBSBAND k2)
{
  ...
  // JD: New flexible way of defining L3 frequency for multi-GNSS
  GOBSBAND b1 = t_gsys::band_priority(gsys, FREQ_1);  GFRQ f1 = t_gsys::freq_priority(gsys, FREQ_1);
  GOBSBAND b2 = t_gsys::band_priority(gsys, FREQ_2);  GFRQ f2 = t_gsys::freq_priority(gsys, FREQ_2);
  GOBSBAND b3 = t_gsys::band_priority(gsys, FREQ_3);  GFRQ f3 = t_gsys::freq_priority(gsys, FREQ_3);
  GOBSBAND b4 = t_gsys::band_priority(gsys, FREQ_4);  GFRQ f4 = t_gsys::freq_priority(gsys, FREQ_4);
  GOBSBAND b5 = t_gsys::band_priority(gsys, FREQ_5);  GFRQ f5 = t_gsys::freq_priority(gsys, FREQ_5);
...
//                       << " " << t_gfreq::gfreq2str(f2) << " " << apcf2 << endl;
   ...
   else
   {
	   if (_gnote)
	   {
		   if (_mappco.find(f1) != _mappco.end()) _gnote->mesg(GWARNING, "gpcv", "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f1) + "], using hardwired values");
		   if (_mappco.find(f2) != _mappco.end()) _gnote->mesg(GWARNING, "gpcv", "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f2) + "], using hardwired values");
		   if (_mappco.find(f3) != _mappco.end()) _gnote->mesg(GWARNING, "gpcv", "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f3) + "], using hardwired values");
		   if (_mappco.find(f4) != _mappco.end()) _gnote->mesg(GWARNING, "gpcv", "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f4) + "], using hardwired values");
		   if (_mappco.find(f5) != _mappco.end()) _gnote->mesg(GWARNING, "gpcv", "no SAT PCO [" + _anten + "/" + t_gfreq::gfreq2str(f5) + "], using hardwired values");
	   }
   ...
   }else if (lc == LC_L3) {apcLC = apcf3;
   }else if (lc == LC_L4) {apcLC = apcf4;
   }else if (lc == LC_L5) {apcLC = apcf5;
   }else {apcLC = apcf2;}
...
// JD: New flexible way of defining L3 frequency for multi-GNSS
   ...
   if (lc == GOBS_LC::LC_L1 || lc == GOBS_LC::LC_L2 || lc == GOBS_LC::LC_L3 || lc == GOBS_LC::LC_L4 || lc == GOBS_LC::LC_L5)
   {
	   ...
	   case GOBS_LC::LC_L2:
		   ...
		   break;
	   case GOBS_LC::LC_L3:
		   if (_mapzen.find(f3) == _mapzen.end()) f3 = f2;
		   if (_mapzen.find(f3) == _mapzen.end()) { _gmutex.unlock(); return -1; }
		   break;
	   case GOBS_LC::LC_L4:
		   if (_mapzen.find(f4) == _mapzen.end()) f4 = f2;
		   if (_mapzen.find(f4) == _mapzen.end()) { _gmutex.unlock(); return -1; }
		   break;
	   case GOBS_LC::LC_L5:
		   if (_mapzen.find(f5) == _mapzen.end()) f5 = f2;
		   if (_mapzen.find(f5) == _mapzen.end()) { _gmutex.unlock(); return -1; }
		   break;
	   default:
			{ _gmutex.unlock(); return -1; }
	   }
   ...
   map<double, double> mapDataf1;  mapDataf1 = _mapzen[f1]; 
   map<double, double> mapDataf2;  mapDataf2 = _mapzen[f2];   
   map<double, double> mapDataf3;  mapDataf3 = _mapzen[f3];   
   map<double, double> mapDataf4;  mapDataf4 = _mapzen[f4];   
   map<double, double> mapDataf5;  mapDataf5 = _mapzen[f5];   
   ...
   else
   {
	   ...
	   case GOBS_LC::LC_L2:
		   ...
		   break;
	   case GOBS_LC::LC_L3:
		   if (interp.linear(mapDataf3, zen, corrf3) < 0) { mapDataf3 = mapDataf2;}
		   if (interp.linear(mapDataf3, zen, corrf3) < 0) { _gmutex.unlock(); return -1; }
		   break;
	   case GOBS_LC::LC_L4:
		   if (interp.linear(mapDataf4, zen, corrf4) < 0) { mapDataf4 = mapDataf2; }
		   if (interp.linear(mapDataf4, zen, corrf4) < 0) { _gmutex.unlock(); return -1; }
		   break;
	   case GOBS_LC::LC_L5:
		   if (interp.linear(mapDataf5, zen, corrf5) < 0) { mapDataf5 = mapDataf2; }
		   if (interp.linear(mapDataf5, zen, corrf5) < 0) { _gmutex.unlock(); return -1; }
		   break;
	   default:
		   {_gmutex.unlock(); return -1; }
	   }
   ...
   // PCV for linear combination
	...
	}else if(lc == LC_L3) {corrLC = corrf3;
	}else if(lc == LC_L4) {corrLC = corrf4;
	}else if(lc == LC_L5) {corrLC = corrf5;}
	else corrLC = corrf2;   
...
//   if(gsys == GAL) gsys = GPS; // JD 2018/09 JUST FOR GALILEO, BELOW FOR ANY
  ...
  // ==================================================
   ...
   bool isAzi = false;
        if (lc == GOBS_LC::LC_IF && _azi_dependent(f1) && _azi_dependent(f2)) isAzi = true;
   else if (lc == GOBS_LC::LC_L1 && _azi_dependent(f1)) isAzi = true;
   else if (lc == GOBS_LC::LC_L2 && _azi_dependent(f2)) isAzi = true;
   else if (lc == GOBS_LC::LC_L3 && _azi_dependent(f3)) isAzi = true;
   else if (lc == GOBS_LC::LC_L4 && _azi_dependent(f4)) isAzi = true;
   else if (lc == GOBS_LC::LC_L5 && _azi_dependent(f5)) isAzi = true;
   else isAzi = _azi_dependent(f2);
   ...
   {  
     ...
     for(map<GFRQ, t_map_A>::iterator itGFRQ = _mapazi.begin(); itGFRQ != _mapazi.end(); itGFRQ++)
	 {
	   ...
	   if (lc == GOBS_LC::LC_IF && (f != f1 && f != f2)) continue;
	   if (lc == GOBS_LC::LC_L1 && (f != f1)) continue;
	   if (lc == GOBS_LC::LC_L2 && (f != f2)) continue;
	   if (lc == GOBS_LC::LC_L3 && (f != f3)) continue;
	   if (lc == GOBS_LC::LC_L4 && (f != f4)) continue;
	   if (lc == GOBS_LC::LC_L5 && (f != f5)) continue;
	 ...
	 else
	 {
		 ...
		 case GOBS_LC::LC_L2:
			 ...
			 break;
		 case GOBS_LC::LC_L3:
			 if (interp.bilinear(mapDataf3, p_az, corrf3) < 0) { _gmutex.unlock(); return -1; }
			 break;
		 case GOBS_LC::LC_L4:
			 if (interp.bilinear(mapDataf4, p_az, corrf4) < 0) { _gmutex.unlock(); return -1; }
			 break;
		 case GOBS_LC::LC_L5:
			 if (interp.bilinear(mapDataf5, p_az, corrf5) < 0) { _gmutex.unlock(); return -1; }
			 break;
		 default:
			 { _gmutex.unlock(); return -1; }
		 }
   ...
   {  // AZI-dependant calibration NOT available (only NOAZI)
	 ...
	 {
		 if (_mapzen.find(f1) != _mapzen.end()) { mapDataf1 = _mapzen[f1]; }
		 if (_mapzen.find(f2) != _mapzen.end()) { mapDataf2 = _mapzen[f2]; }
		 if (_mapzen.find(f3) != _mapzen.end()) { mapDataf3 = _mapzen[f3]; }
		 if (_mapzen.find(f4) != _mapzen.end()) { mapDataf4 = _mapzen[f4]; }
		 if (_mapzen.find(f5) != _mapzen.end()) { mapDataf5 = _mapzen[f5]; }

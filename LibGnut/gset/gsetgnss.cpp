...
namespace gnut
{
	...
	set<GSYS> t_gsetgnss::SYS_SUPPORTED()
	{
		set<GSYS> systems;
	...
	double t_gsetgnss::_sigma_D(GSYS gsys)
	{
		...
		// default value
		if (double_eq(sig, 0.0)) sig = _sigma_def_doppler[gsys];  // map<sys,double>
	...
	double t_gsetgnss::_maxres_D(GSYS gsys)
	{
		...
		// default value
		if (double_eq(sig, 0.0)) sig = _maxres_def_doppler[gsys];   // map<sys,double>

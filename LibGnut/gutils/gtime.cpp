...
namespace gnut {
	...
	{
		if (y % 4 == 0.0 && y % 100 != 0.0) return 1; //     leap year !
	...
	int t_gtime::gwk() const
	{
		...
		return static_cast<int>((_mjd_conv - 44244.0) / 7.0);
	}
	...
	int t_gtime::dow() const
	{
		...
		return static_cast<int>(_mjd_conv - 44244.0 - gwk() * 7);
	}
	...
	{
		return ((_mjd_conv < t._mjd_conv)
			|| (_mjd_conv == t._mjd_conv && _sod_conv < t._sod_conv)
	...
	void t_gtime::_reset_conv()
	{
		...
		while (sod_temp >= 86400) { sod_temp -= 86400; }
		while (sod_temp < 0) { sod_temp += 86400; }
		_sod_conv = int(sod_temp);

		_dsec_conv = _dsec + tai_tsys_dsec(_tsys);
		while (_dsec_conv >= 1.0) { _dsec_conv -= 1.0; }
		while (_dsec_conv < 0.0) { _dsec_conv += 1.0; }
	...
	{
		t_gtime tmax;
		if (A.empty()) return tmax;
		tmax = A[0];
		for (const auto& iter : A) {
			if (tmax < iter) tmax = iter;
		}
		return tmax;
	}
	...
	{
		t_gtime tmin;
		if (A.empty()) return tmin;
		tmin = A[0];
		for (const auto& iter : A) {
			if (tmin > iter) tmin = iter;
		}
		return tmin;
	}

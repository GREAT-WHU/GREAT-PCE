...
namespace great
{
	...
	/** map container using station name as a index for storaging t_vec_ambflag data of all station */
	typedef map<string, t_gambflag>  t_map_allambflag;

	/**
	*@brief	   Class for all XXXXddd0.yyo.log/log13 file data storaging
	*/
	class LibGnut_LIBRARY_EXPORT t_gallambflag : public t_gdata
	{
	...
	public:
		...
		*/
		...
		void clearSiteAmbFlag(string site) { _map_allambflag[site].clearAllAmbFlagData(); };

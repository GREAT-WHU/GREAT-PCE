...
t_gnavqzs::t_gnavqzs()
  : t_gnav(),
    ...
    _flag(0.0)
{ 
  gtrace("t_gnavqzs::t_gnavqzs");
   
  _tgd[0] = _tgd[1] = _tgd[2] = _tgd[3] = 0.0;

  id_type(t_gdata::EPHQZS);
...
// print function
// ----------
string t_gnavqzs::line() const
{
  gtrace("t_gnavqzs::line");
...
// print function
// ----------
string t_gnavqzs::linefmt() const
{
  gtrace("t_gnavqzs::str");
...
// ----------
int t_gnavqzs::chk(set<string>& msg)
{
  gtrace("t_gnavqzs::chk");   
  _gmutex.lock(); 
  ...
  if( (_tot - _toc) > 86400 || (_toc - _tot) > t_gnav::nav_validity(QZS)){
    msg.insert("Issue: "+_sat+" nav large difference [tot-toc] " + dbl2str(_tot-_toc) + " s " + _tot.str_ymdhms() + _toc.str_ymdhms(" "));
    _validity = false;
  }

  if( (_tot - _toe) > 86400 || (_toe - _tot) > t_gnav::nav_validity(QZS)){
...
// ----------
int t_gnavqzs::pos( const t_gtime& t, double xyz[], double var[], double vel[], bool chk_health )
{
  gtrace("t_gnavqzs::pos");

  if( sat().empty() ) return -1;  // not valid !!!
  if( chk_health && _healthy() == false ){
     if( _log ) _log->comment(3,"gephqzs","not healthy sat " + sat() + " excluded from pos calculation " + t.str_ymdhms());
  ...
  while( Tk >  302400.0 ){ Tk -= 604800.0; }        // check the correct QZSS week
  while( Tk < -302400.0 ){ Tk += 604800.0; }        // check the correct QZSS week
...
// ----------
int t_gnavqzs::clk( const t_gtime& t, double* clk, double* var, double* dclk, bool chk_health )
{
  gtrace("t_gnavqzs::clk");

  if( sat().empty() ) return -1;  // not valid !!!

  if( chk_health && _healthy() == false ){
     if( _log ) _log->comment(3,"gephqzs","not healthy sat " + sat() + " excluded from clk calculation " + t.str_ymdhms());
     return -1;  // HEALTH NOT OK
  }
...
// ----------
int t_gnavqzs::data2nav( string sat, const t_gtime& ep, const t_gnavdata& data )
{
  gtrace("t_gnavqzs::data2nav");   
  ...
  if( fabs(_tot - _toe - 604800) < MAX_QZS_TIMEDIFF ) _tot.add_secs(-604800);   // adjust tot to toe gwk
  if( fabs(_tot - _toe + 604800) < MAX_QZS_TIMEDIFF ) _tot.add_secs(+604800);   // adjust tot to toe gwk
...
// ----------
int t_gnavqzs::nav2data( t_gnavdata& data )
{
...
bool t_gnavqzs::chktot(const t_gtime& t)
{
...
// get parameter value
// ----------
t_timdbl t_gnavqzs::param( const NAVDATA& n )
{
  _gmutex.lock();
  ...
  switch (n){
   ...
   case NAV_TGD0  : tmp = make_pair(_toc,_tgd[0] *1e0);  break; // sec
   case NAV_TGD1  : tmp = make_pair(_toc,_tgd[1] *1e0);  break; // sec
   case NAV_TGD2  : tmp = make_pair(_toc,_tgd[2] *1e0);  break; // sec
   case NAV_TGD3  : tmp = make_pair(_toc,_tgd[3] *1e0);  break; // sec
...
// set parameter value
// ----------
int t_gnavqzs::param( const NAVDATA& n, double val )
{
  _gmutex.lock();
  ...
  switch (n){       // SELECTED only, ! use the same MULTIPLICATOR as in param()
   ...
   case NAV_TGD0  : _tgd[0] = val/1.e0;  break;
   case NAV_TGD1  : _tgd[1] = val/1.e0;  break;
   case NAV_TGD2  : _tgd[2] = val/1.e0;  break;
   case NAV_TGD3  : _tgd[3] = val/1.e0;  break;
...
// healthy check
// ----------
bool t_gnavqzs::_healthy() const
{  
  string health_bin = bitset<5>(_health).to_string();
  reverse(health_bin.begin(), health_bin.end());

  // only L1 C/A is checked now
  if( health_bin.compare(0,1,"0") == 0) return true;
  return false;
}
...
// mean motion
// ----------
double t_gnavqzs::_mean_motion()
{
  gtrace("t_gnavqzs::_mean_motion");   
...
// eccentric anomaly
// ----------
void t_gnavqzs::_ecc_anomaly( double dt, double& Ek, double& dEk )
{
  gtrace("t_gnavqzs::_ecc_anomaly");   

...
t_gnavirn::t_gnavirn()
  : t_gnav(),
    ...
    _acc(0.0)
{ 
  gtrace("t_gnavirn::t_gnavirn");
   
  _tgd[0] = _tgd[1] = _tgd[2] = _tgd[3] = 0.0;

  id_type(t_gdata::EPHIRN);
...
// print function
// ----------
string t_gnavirn::line() const
{
  gtrace("t_gnavirn::line");   
...
// print function
// ----------
string t_gnavirn::linefmt() const
{
  gtrace("t_gnavirn::str");
...
// ----------
int t_gnavirn::chk(set<string>& msg)
{
  gtrace("t_gnavirn::chk");   
  _gmutex.lock(); 
  ...
  if( (_tot - _toc) > 86400 || (_toc - _tot) > t_gnav::nav_validity(IRN)){
    msg.insert("Issue: "+_sat+" nav large difference [tot-toc] " + dbl2str(_tot-_toc) + " s " + _tot.str_ymdhms() + _toc.str_ymdhms(" "));
    _validity = false;
  }
  
  if( (_tot - _toe) > 86400 || (_toe - _tot) > t_gnav::nav_validity(IRN)){
...
// ----------
int t_gnavirn::pos( const t_gtime& t, double xyz[], double var[], double vel[], bool chk_health )
{
  gtrace("t_gnavirn::pos");
...
int t_gnavirn::clk( const t_gtime& t, double* clk, double* var, double* dclk, bool chk_health )
{
  gtrace("t_gnavirn::clk");
...
int t_gnavirn::data2nav( string sat, const t_gtime& ep, const t_gnavdata& data )
{
  gtrace("t_gnavirn::data2nav");
  ...
  if( fabs(_tot - _toe - 604800) < MAX_IRN_TIMEDIFF ) _tot.add_secs(-604800);   // adjust tot to toe gwk
  if( fabs(_tot - _toe + 604800) < MAX_IRN_TIMEDIFF ) _tot.add_secs(+604800);   // adjust tot to toe gwk
...
// ----------
int t_gnavirn::nav2data( t_gnavdata& data )
{
  gtrace("t_gnavirn::nav2data");
...
bool t_gnavirn::chktot(const t_gtime& t)
{
...
// get parameter value
// ----------
t_timdbl t_gnavirn::param( const NAVDATA& n )
{
  _gmutex.lock();
...
// set parameter value
// ----------
int t_gnavirn::param( const NAVDATA& n, double val )
{
  _gmutex.lock();
...
// mean motion
// ----------
double t_gnavirn::_mean_motion()
{
  gtrace("t_gnavirn::_mean_motion");   
...
// eccentric anomaly
// ----------
void t_gnavirn::_ecc_anomaly( double dt, double& Ek, double& dEk )
{
  gtrace("t_gnavirn::_ecc_anomaly");   

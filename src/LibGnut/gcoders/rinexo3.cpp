...
int t_rinexo3::_fix_band(string sys, string& go)
{
  ...
  if( t_gsys::str2gsys(sys) == BDS )
  {
	  ...
	  {
		  go[1] = '2';
		  if (_log) _log->comment(0, "rinexo3", 
			  "Warning: Both C1x and C2x coding should be accepted and treated as C2x in RINEX 3.02/3.03");
		  else   cerr << "rinexo3 - Warning: Both C1x and C2x coding should be accepted and treated as C2x in RINEX 3.02/3.03\n";
		  mesg(GWARNING, "Warning: BDS band changed: C1x->C2x for RINEX " + _version);
	  
	  }
	  if (go[1] == '3') 
	  {
		go[1] = '6'; // for B3I (change C3I -> C6I or so on !!!)
		if (_log) _log->comment(0, "rinexo3", "Warning: BDS band changed: C3x -> C6x ");
		else               cerr << "rinexo3 - Warning: BDS band changed: C3x -> C6x\n";
		mesg(GWARNING, "Warning: BDS band changed: C3x -> C6x " + _version);
	  }
	  if (go[1] == '7' && _version >= "3.04")
	  {
		  ...
		  if (_log) _log->comment(0, "rinexo3",
			  "Warning: Changing BDS-3 C7D/C7P/C7Z to C9D/C9P/C9Z in RINEX 3.04");
		  else   cerr << "rinexo3 - Warning: Changing BDS-3 C7D/C7P/C7Z to C9D/C9P/C9Z in RINEX 3.04\n";
		  mesg(GWARNING, "Warning: Changing BDS-3 C7D/C7P/C7Z to C9D/C9P/C9Z " + _version);
	  }
  } 

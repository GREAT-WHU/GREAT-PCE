  ...
  if( _ss_position == 0 ) {
    ...
    for( it = _data.begin(); it != _data.end(); ++it ) {
      if( it->second->id_type()  == t_gdata::ALLOBJ ) {
		  ...
		  if (allGrec.find(_encode_site) != allGrec.end()){
			  shared_ptr<t_grec> grecCasted = dynamic_pointer_cast<t_grec> (allGrec[_encode_site]);
              ...
              if(hdrFromGrec.begin() != hdrFromGrec.end()) {
                  ...
                  for(mapobsIt = mapobs.begin(); mapobsIt != mapobs.end(); ++mapobsIt) {
                      ...
                      for(odfpIt = obsDescriptorFactorPairs.begin(); odfpIt != obsDescriptorFactorPairs.end();
						  ...
						  t_gobs gobs_type(gobs); 
						  gobs_type.gobs2to3(t_gsys::str2gsys(satSys));
						  GOBS gobs1 = gobs_type.gobs();
    ...
    for( it = _data.begin(); it != _data.end(); ++it ) {
        if( it->second->id_type()  == t_gdata::ALLOBS ) {
         ...
         // string markname = *(allStations.begin());
		  if (allStations.find(_encode_site) == allStations.end())  continue;
		  string markname = _encode_site;
